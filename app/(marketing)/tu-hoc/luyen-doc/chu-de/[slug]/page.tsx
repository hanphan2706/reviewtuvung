...
import { readingLibraryByTopic } from "@/lib/reading/library-nav";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const page = readingLibraryByTopic(slug);
...
export default async function ReadingLibraryByTopicPage({ params }: PageProps) {
  const { slug } = await params;
  const page = readingLibraryByTopic(slug);
  if (!page) notFound();
