...
import { listeningLibraryBySource } from "@/lib/listening/library-nav";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const page = listeningLibraryBySource(slug);
  if (!page) {
...
export default async function ListeningLibraryBySourcePage({ params }: PageProps) {
  const { slug } = await params;
  const page = listeningLibraryBySource(slug);
  if (!page) notFound();
