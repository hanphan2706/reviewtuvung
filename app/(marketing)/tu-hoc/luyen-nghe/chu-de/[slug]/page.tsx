...
import { listeningLibraryByTopic } from "@/lib/listening/library-nav";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const page = listeningLibraryByTopic(slug);
  if (!page) {
...
export default async function ListeningLibraryByTopicPage({ params }: PageProps) {
  const { slug } = await params;
  const page = listeningLibraryByTopic(slug);
  if (!page) notFound();
