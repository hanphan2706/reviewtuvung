import { EVIU_ADVANCED_CATALOG } from "../eviu-advanced-catalog";
...
import { listEviuUpperIntermediateUnits } from "./eviu-upper-intermediate-all-units";

export function catalogFor(unitNumber: number) {
  const entry = EVIU_ADVANCED_CATALOG.find((item) => item.unitNumber === unitNumber);
  if (!entry) {
    throw new Error(`Missing advanced catalog entry for unit ${unitNumber}`);
  }
  return entry;
}
...
/** Lemma/cụm đã có ở Elementary, Pre-Int hoặc Upper-Int — bỏ khi ship Advanced. */
export function getLowerLevelVocabularyKeys(): ReadonlySet<string> {
  ...
  for (const unit of listEviuUpperIntermediateUnits()) {
    for (const word of unit.words) {
      const key = normalizeVocabularyKey(word.term);
      if (key) keys.add(key);
    }
...
export function advancedUnit(unitNumber: number, content: UnitContentInput): VocabularyUnit {
