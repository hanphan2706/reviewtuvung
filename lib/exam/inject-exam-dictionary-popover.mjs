...
.exam-review-dict-hint{
  display:inline-block;margin:0 12px 0 8px;padding:10px 8px 10px 0;font-size:13px;font-weight:600;line-height:1.35;
  font-style:italic;color:#71717A;flex:0 0 auto;text-align:right;white-space:nowrap
}
...
function buildDictionaryPopoverScript(kind) {
  ...
  const hintText =
    kindLiteral === "listening"
      ? "Bạn có thể tra từ và thêm từ vào deck ở đây nha."
      : "*Bạn có thể tra từ và thêm từ vào deck ngay từ bài đọc.";
...
(function(){
  ...
  function canShowAdd(){
    ...
    var isTranslateOnly=lookup.phraseGlossVi&&(!lookup.senses||!lookup.senses.length);
    if(isTranslateOnly)return false;
    return lookup.kind==="word"||(lookup.kind==="phrase"&&lookup.phraseGlossVi);
  }
  ...
  function renderFooter(){
    ...
    if(phase==="pick-deck"){
      ...
      next.addEventListener("click",function(){
        ...
        term=lookup.query;
        definition=formatLookupForDeck(lookup);
        phase="add";
        renderAddForm();
      });
