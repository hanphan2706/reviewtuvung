import type { DictDefinition, DictEntry, DictMeaning, DictPhonetic } from "@/lib/reading/lookup-internal-types";
...
export function scoreDefinition(def: DictDefinition): number {
...
export function collectScoredDefinitions(entries: DictEntry[], limit: number): ScoredDefinition[] {
  ...
  for (const entry of entries) {
    for (const meaning of entry.meanings ?? []) {
      ...
      for (let i = 1; i < defs.length; i++) {
        const en = defs[i]?.definition?.trim();
        if (!en || en.length < 8 || seen.has(en.toLowerCase()) || isTechnicalDefinition(en)) continue;
        if (primaryDefs.has(en.toLowerCase())) continue;
