/**
 * Whisper Cam 16 Test 1–4 → transcript + sync (local faster-whisper).
 *
 *   npx tsx scripts/sync-cam16-listening.ts
 *   npx tsx scripts/sync-cam16-listening.ts --test 1
 */
...
function resolveAudio(test: number, part: number): string {
  const file = listeningPartAudioFileName("cam16", test, part);
...
async function syncPart(test: number, part: number, useCache: boolean): Promise<string> {
  const partId = `cam16-t${test}-p${part}`;
  const audioPath = resolveAudio(test, part);
  const durationSeconds = probeDurationSeconds(audioPath);
  console.log(`[cam16] Whisper ${partId} (${durationSeconds.toFixed(1)}s)`);
  ...
  const sync = buildWhisperSegmentOnlyTranscriptSync({
    partId,
    audioFile: listeningPartAudioFileName("cam16", test, part),
    durationSeconds,
...
async function main(): Promise<void> {
  ...
  for (const test of tests) {
    ...
    const dest = path.join(transcriptDir, `cam16-test ${test}.whisper.txt`);
    writeFileSync(dest, `${blocks.join("\n\n")}\n`);
    console.log(`[cam16] wrote ${dest} (does not overwrite book transcript)`);
  }
}
