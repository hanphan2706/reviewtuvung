/**
 * Align Cam 14 book transcripts (with Q markers) to Whisper word timings.
 * Uses cached `*.whisper-words.json` when present.
 *
 *   npx tsx scripts/sync-cam14-listening.ts
 *   npx tsx scripts/sync-cam14-listening.ts --test 1
 *   npx tsx scripts/sync-cam14-listening.ts --test 1 --part 4
 */
...
function resolveAudio(test: number, part: number): string {
  const file = listeningPartAudioFileName("cam14", test, part);
...
function loadPlainTranscript(test: number, part: number): string {
  ...
  const candidates = [
    path.join(transcriptDir, `cam14-test${test}.cleaned.txt`),
    path.join(transcriptDir, `cam14-test ${test}.txt`),
  ];
  const found = candidates.find((p) => fs.existsSync(p));
  if (!found) throw new Error(`Missing transcript for cam14 test ${test}`);
...
async function syncPart(test: number, part: number, useCache: boolean): Promise<void> {
  const partId = `cam14-t${test}-p${part}`;
  const audioPath = resolveAudio(test, part);
  const audioFile = listeningPartAudioFileName("cam14", test, part);
  const durationSeconds = probeDurationSeconds(audioPath);
  const plain = loadPlainTranscript(test, part);

  console.log(`[cam14] Align ${partId} (${durationSeconds.toFixed(1)}s) → book transcript + Whisper words`);
