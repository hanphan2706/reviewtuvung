/**
 * Upload only Cam 14 listening MP3s to Supabase Storage.
 *
 *   npx tsx scripts/upload-cam14-listening-audio.ts
 */
import { readdirSync, readFileSync } from "node:fs";
import path from "node:path";
import { loadEnvLocal } from "./load-env-local";
import { LISTENING_AUDIO_BUCKET } from "../lib/listening/listening-audio-storage";
import { LISTENING_AUDIO_CAM14_SUBDIR } from "../lib/listening/listening-materials-paths";
...
async function main(): Promise<void> {
  ...
  const dir = path.join(process.cwd(), LISTENING_AUDIO_CAM14_SUBDIR);
  const names = readdirSync(dir)
    .filter((name) => /^cam14-test[1-4]-part[1-4]\.mp3$/i.test(name))
    .sort();

  if (names.length !== 16) {
    console.error(`Expected 16 Cam 14 mp3 files, found ${names.length} in ${dir}`);
    process.exit(1);
  }
