/**
 * Upload only Cam 12 listening MP3s to Supabase Storage.
 *
 *   npx tsx scripts/upload-cam12-listening-audio.ts
 */
import { readdirSync, readFileSync } from "node:fs";
import path from "node:path";
import { loadEnvLocal } from "./load-env-local";
import { LISTENING_AUDIO_BUCKET } from "../lib/listening/listening-audio-storage";
import { LISTENING_AUDIO_CAM12_SUBDIR } from "../lib/listening/listening-materials-paths";
...
async function main(): Promise<void> {
  ...
  const dir = path.join(process.cwd(), LISTENING_AUDIO_CAM12_SUBDIR);
  const names = readdirSync(dir)
    .filter((name) => /^cam12-test[1-4]-part[1-4]\.mp3$/i.test(name))
    .sort();

  if (names.length !== 16) {
    console.error(`Expected 16 Cam 12 mp3 files, found ${names.length} in ${dir}`);
    process.exit(1);
  }
